...
# Date
# local by default, a UTC value now shows the device's local time
- type: component.entity-field
  options:
    label: Updated
    value:
      text: =@ctx.current.item.updatedAt # 2026-07-18T12:00:00Z
      format:
        dateFormat: LT # -> 2:00 PM at UTC+2 (was 12:00 PM)

# explicit timezone, DST-correct for the formatted date
- type: component.entity-field
  options:
    label: Store hours (Prague)
    value:
      text: =@ctx.current.item.opensAt
      format:
        dateFormat: HH:mm
        timezone: Europe/Prague # IANA name, or 'local', or 'UTC'              
```
{% endcode %}
