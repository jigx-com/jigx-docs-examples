...
<table><thead><tr><th width="213.6875">Action</th><th></th></tr></thead><tbody><tr><td><code>onBlur</code></td><td><code>onBlur</code> is triggered when you leave the field, for example, when you tap into another field, close the picker, or dismiss the keyboard. It allows you to run actions after the user finishes interacting with a field, rather than on every value change. </td></tr><tr><td><code>onChange</code></td><td>Select an action to execute when the date-picker component's value is changed.</td></tr></tbody></table>
